...
public:
    ...
    void Begin(uint8_t count)
    {
        if (count > _alarmsCount)
        {
            ...
            _msLast = to_ms_since_boot(get_absolute_time());
            _seconds = 0;
        }
    }
    ...
    {
        uint32_t msNow = to_ms_since_boot(get_absolute_time());
    ...
    void ProcessAlarms(RtcAlarmCallback callback, void* context)
    {
        ...
        if (msDelta > 1000)
        {
            // update seconds based on passed time using to_ms_since_boot(get_absolute_time())
...
protected:
    ...
    uint32_t _msLast; // the last call to to_ms_since_boot(get_absolute_time())
