...
const uint8_t DS1302Tcr_Disabled =
    std::to_underlying(DS1302TcrStatus_Disabled) | std::to_underlying(DS1302TcrDiodes_Disabled) | std::to_underlying(DS1302TcrResistor_Disabled);
...
public:
  ...
  bool IsDateTimeValid() { return GetDateTime().IsValid(); }

  bool GetIsRunning() {
    uint8_t ch = getReg(DS1302_REG_CH);
    return (ch & _BV(DS1302_CH)) == 0U;
  }
  ...
  void SetTrickleChargeSettings(uint8_t setting) {
    if ((setting & DS1302TcrResistor_MASK) == DS1302TcrResistor_Disabled || (setting & DS1302TcrDiodes_MASK) == DS1302TcrDiodes_Disabled ||
        (setting & DS1302TcrDiodes_MASK) == DS1302TcrDiodes_None || (setting & DS1302TcrStatus_MASK) != DS1302TcrStatus_Enabled) {
  ...
  uint8_t GetMemory(uint8_t memoryAddress) {
    ...
    // memory addresses interleaved read and write addresses
    // so we need to calculate the offset
    if (uint8_t address = (memoryAddress * 2) + DS1302_REG_RAMSTART; address <= DS1302_REG_RAMEND) {
