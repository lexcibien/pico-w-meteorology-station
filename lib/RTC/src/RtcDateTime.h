...
class RtcHourAmPm {
public:
  RtcHourAmPm() : _hour(12), _meridiem(Rtc_AM) {}
...
class RtcDateTime {
public:
  explicit RtcDateTime(uint32_t secondsFrom2000 = 0) { _initWithSecondsFrom2000<uint32_t>(secondsFrom2000); }

  RtcDateTime(uint16_t year, uint8_t month, uint8_t dayOfMonth, uint8_t hour, uint8_t minute, uint8_t second)
      : _yearFrom2000((year >= c_OriginYear) ? year - c_OriginYear : year), _month(month), _dayOfMonth(dayOfMonth), _hour(hour), _minute(minute),
        _second(second) {}

  // RtcDateTime compileDateTime(__DATE__, __TIME__);
  // sample input: date = "Dec 06 2009", time = "12:34:56"
  RtcDateTime(const char* date, const char* time) : RtcDateTime(0) {
  ...
  */
  ...
  // add signed seconds
  void operator+=(int32_t seconds) { *this = *this + seconds; }
  ...
  // remove seconds
  void operator-=(uint32_t seconds) { *this = *this - seconds; }

  RtcDateTime operator-(uint32_t seconds) const { return RtcDateTime(TotalSeconds() - seconds); }
  ...
  template <typename T_LOCALE = RtcLocaleEnUs> size_t InitWithDateTimeFormatString(const char* format, const char* datetime) {
    ...
    while (*scan != '\0' && *datetime != '\0') {
      ...
      if (*scan != '\0') {
        ...
        switch (scan[iStart]) {
          ...
          case 'D': countConverted = CharsToNumber<uint8_t>(convert, &_dayOfMonth, count); break;

          case 'h': countConverted = CharsToNumber<uint8_t>(convert, &_hour, count); break;

          case 'm': countConverted = CharsToNumber<uint8_t>(convert, &_minute, count); break;

          case 's': countConverted = CharsToNumber<uint8_t>(convert, &_second, count); break;
  ...
  static uint8_t ConvertRtcToDow(uint8_t rtcDow) { return (rtcDow % 7); }
  ...
  static uint8_t DaysInMonth(uint16_t year, uint8_t month) {
    ...
    uint8_t days = c_daysInMonth[zMonth];
